...
const char HTTP_PORTAL_OPTION[] PROGMEM  = "<form action='{x}' method='get'> <button>{y}</button></form><br/>"; // yurij
const char HTTP_HOME_BUTTON[] PROGMEM    = "<br><form action='/' method='post'><button>Home</button></form>";   // yurij
...
#define WIFI_MANAGER_MAX_BUTTONS 10
...
class WiFiManager
{
  ...
  public:
    void    addButton(const char* name, const char* path);
    void    setLoopCallback( void (*func)(void) );
    void    setUserHandlers( void (*func)(void) );
    void    setWebServerHandlers( void );    // yurij
  protected:
    WiFiManagerButton*  _buttons[WIFI_MANAGER_MAX_BUTTONS];
    int     _buttonCount = 0;

    void    (*_apLoopCallback)( void ) = NULL;
    void    (*_apUserHandlers)( void ) = NULL;
};
